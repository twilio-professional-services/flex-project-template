...
class ChatTransferService extends ApiService {
  ...
  // is completed
  async executeChatTransfer(task: Flex.ITask, transferTargetSid: string, options?: TransferOptions): Promise<CreateTransferTaskResponse> {
    try {
      ...
      const queueResult = queues? queues.find(queue => {
      ...
      const { success, taskSid: transferTaskSid, message } = await this.#createTransferTask(task, transferTargetSid, queueName);
      if (success) {
  ...
  async completeTransferredTask(task: Flex.ITask): Promise<Boolean> {
    ...
    try {
      const { attributes: { channelSid, chatTransferData: { transferType } }, taskSid } = task;
  ...
  #createTransferTask = (task: Flex.ITask, transferTargetSid: string, queueName: string): Promise<CreateTransferTaskResponse> => {
    ...
    return this.fetchJsonWithReject<CreateTransferTaskResponse>(
      `https://${this.serverlessDomain}/functions/features/chat-transfer/flex/create-transfer-task`,
      {
        method: 'post',
  ...
  #completeTransferredTask = (taskSid: string, transferType: string, channelSid: string): Promise<CompleteTransferredTaskResponse> => {
    ...
    return this.fetchJsonWithReject<CompleteTransferredTaskResponse>(
      `https://${this.serverlessDomain}/functions/features/chat-transfer/flex/complete-task-for-transfer`,
      {
        method: 'post',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: this.buildBody(encodedParams)
