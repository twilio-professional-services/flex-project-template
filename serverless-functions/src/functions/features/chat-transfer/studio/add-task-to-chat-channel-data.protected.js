...
exports.handler = async function addTaskToChatChannelData(context, event, callback) {
  ...
  if (parameterError) {
    ...
    try {
      ...
      const { success } = await ChatOperations.addTaskToChannel({scriptName, context, taskSid, channelSid, attempts: 0});
