...
const ChatOperations = require(Runtime.getFunctions()['functions/features/chat-transfer/common/chat-operations'].path);

exports.handler = TokenValidator(async function createTransferTask(context, event, callback) {

  const scriptName = arguments.callee.name;
  const response = new Twilio.Response();
  const requiredParameters = [
    { key: 'conversationId', purpose: 'conversation_id to link tasks for reporting' },
    { key: 'jsonAttributes', purpose: 'JSON calling tasks attributes to perpetuate onto new task' },
    { key: 'transferTargetSid', purpose: 'sid of target worker or target queue' },
    { key: 'transferQueueName', purpose: 'name of the queue if transfering to a queue, otherwise empty string' },
    { key: 'ignoreWorkerContactUri', purpose: 'woker Contact Uri to ignore when transfering' },
  ];
  ...
  if (parameterError) {
    ...
    try {
      ...
      const {
        ...
        priority: overriddenPriority } = event;
      ...
      const {
        ...
        task: newTask,
        status: createTaskStatus } = await TaskOperations.createTask({scriptName, context, workflowSid, taskChannel: 'chat', attributes: newAttributes, priority, timeout, attempts: 0});
      ...
      try {
        if (createTaskSuccess) await ChatOperations.addTaskToChannel({scriptName, context, taskSid: newTask.sid, channelSid: newTask.attributes.channelSid, attempts: 0});
      } catch (error) {
