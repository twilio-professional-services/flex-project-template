...
exports.handler = TokenValidator(async function completeTaskForTransfer(context, event, callback) {
  ...
  if (parameterError) {
    ...
    try {
      ...
      const { success: removeSidSuccess, message } = await ChatOperations.removeChannelSidFromTask({scriptName, context, taskSid, attempts: 0});
      ...
      try {
        await ChatOperations.setTaskToCompleteOnChannel({scriptName, context, taskSid, channelSid, attempts: 0});
      } catch (error) {
      ...
      const { success: completeTaskSuccess, message: completeTaskMessage } = await TaskOperations.completeTask(context, taskSid, reason, 0);
