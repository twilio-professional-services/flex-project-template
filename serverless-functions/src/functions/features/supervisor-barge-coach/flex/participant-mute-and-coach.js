...
const ConferenceOperations = require(Runtime.getFunctions()['functions/common/twilio-wrappers/conference-participant'].path);
...
exports.handler = TokenValidator(async function participantMuteCoach(context, event, callback) {
  ...
  const requiredParameters = [
    ...
    { key: 'participantSid', purpose: 'participant sid to target for conference changes' },
    { key: 'agentSid', purpose: 'sid of target worker' },
    { key: 'muted', purpose: 'boolean to determine the muted status of the participant in the conference' },
    { key: 'coaching', purpose: 'boolean to determine the coaching status of the participant in the conference' },
  ];
