...
exports.handler = async function list_phone_numbers(context, event, callback) {
  const scriptName = arguments.callee.name;
	...
	try {
		const result = await PhoneNumberOpertions.listPhoneNumbers({scriptName, context, attempts: 0});
		const { success, phoneNumbers: fullPhoneNumberList, status } = result;
		const phoneNumbers = fullPhoneNumberList? fullPhoneNumberList.map(number => {
