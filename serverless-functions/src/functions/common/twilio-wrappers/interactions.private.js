...
exports.participantCreateInvite = async function participantCreateInvite(parameters) {
  ...
  return twilioExecute(context, (client) =>
    client.flexApi.v1.interaction(interactionSid).channels(channelSid).invites.create({
      routing,
    }),
  );
};
...
exports.participantUpdate = async function participantUpdate(parameters) {
  ...
  return twilioExecute(context, (client) =>
    client.flexApi.v1.interaction(interactionSid).channels(channelSid).participants(participantSid).update({ status }),
  );
};
