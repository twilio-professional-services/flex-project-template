...
const TaskRouterOperations = require(Runtime.getFunctions()['functions/common/twilio-wrappers/taskrouter'].path);

exports.handler = TokenValidator(async function updateWorkerChannel(context, event, callback) {
  const scriptName = arguments.callee.name;
	const response = new Twilio.Response();
  const requiredParameters = [
    { key: 'workerSid', purpose: 'unique ID of the worker' },
    { key: 'workerChannelSid', purpose: 'unique ID of the workerChannelSid to update' },
  ...
  if (parameterError) {
    ...
    try {
      ...
      const result = await TaskRouterOperations.updateWorkerChannel({scriptName, context, attempts: 0, workerSid, workerChannelSid, capacity: Number(capacity), available: available==='true'});
