...
const ChatOperations = require(Runtime.getFunctions()['functions/common/twilio-wrappers/programmable-chat'].path);

exports.handler = TokenValidator(async function updateChannelAttributes(context, event, callback) {
  ...
  if (parameterError) {
    ...
    try {
      ...
      const result = await ChatOperations.updateChannelAttributes({scriptName,context, channelSid, attributes, attempts: 0});
