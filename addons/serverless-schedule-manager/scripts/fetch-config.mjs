...
import { setAccount, fetchServerlessDomains, getFetchedVars } from '../../../scripts/common/fetch-cli.mjs';
...
if (!apiKey || !apiSecret || !accountSid) {
  ...
  if (!apiKey || !apiSecret) {
    throw new Error('Please set the TWILIO_ACCOUNT_SID, TWILIO_API_KEY, and TWILIO_API_SECRET environment variables');
  }
}
...
const fetchConfig = async () => {
  // Fetch domain via API
  setAccount({
    apiKey,
    apiSecret,
    accountSid,
  });
  await fetchServerlessDomains();
