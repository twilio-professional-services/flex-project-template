...
:::tip
If you wish to use this feature with outbound calling (for example, to allow a worker to receive an inbound call while already handling an outbound call), Flex will display a warning dialog, even though this feature can handle the scenario successfully. To disable this warning dialog, please contact Twilio Support to enable a feature flag on your account.
:::
