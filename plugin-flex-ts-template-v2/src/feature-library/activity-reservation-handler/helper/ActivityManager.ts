...
class ActivityManager {
  ...
  updateState = async (overrideCurrentState?: boolean) => {
  ...
  #tryNext = async () => {
    if (!this.currentRequests.length) {
      ...
      const { resolve, reject, overrideCurrentState } = this.currentRequests.shift() as FunctionPromise;
      this.runningRequests += 1;
      const req = this.#updateState(overrideCurrentState);
      req
  ...
  #updateState = async (overrideCurrentState?: boolean) => {
    ...
    // overrideCurrentstate is used when manually selecting the activity from the agent drop down
    const acdAvailabilityStatus =
      overrideCurrentState === undefined ? currentWorkerActivity?.available || false : overrideCurrentState;
