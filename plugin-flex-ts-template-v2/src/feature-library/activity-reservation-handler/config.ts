...
const { enabled = false, system_activity_names } = getFeatureFlags()?.features?.activity_reservation_handler as ActivityReservationHandlerConfig || {};
...
const validateAndSetWorkerActivity = () => {
  ...
  if (FlexHelper.otherSessionDetected) {
    console.warn(
      "Another flex session was detected. " +
  ...
  const { activitySid: workerActivitySid, activityName: workerActivityName } =
    WorkerActivity;
  ...
  if (!FlexHelper.hasLiveCallTask && FlexHelper.hasWrappingTask) {
    ...
    console.log(
      `Resetting "${targetActivity?.name}" Activity from:`,
      workerActivityName
    );

    WorkerActivity.setWorkerActivity(targetActivity?.sid);
  } else if (
    (workerActivitySid === wrapupActivity?.sid ||
      workerActivitySid === wrapupNoAcdActivity?.sid) &&
    !FlexHelper.hasWrappingTask
  ) {
    const targetActivity = pendingActivity
      ? pendingActivity
      : availableActivity;

    console.log(
      `Setting worker from "${workerActivityName}" to ` +
        `${pendingActivity ? "pending" : "default"} activity:`,
      targetActivity?.name
    );

    WorkerActivity.setWorkerActivity(
      targetActivity?.sid,
      pendingActivity ? true : false
    );
  } else if (
    (workerActivitySid === onTaskActivity?.sid ||
      workerActivitySid === onTaskNoAcdActivity?.sid) &&
    ...
    workerActivitySid === FlexHelper.offlineActivitySid &&
    !FlexHelper.hasWrappingTask
  ) {
    clearPendingActivityChange();
  }
