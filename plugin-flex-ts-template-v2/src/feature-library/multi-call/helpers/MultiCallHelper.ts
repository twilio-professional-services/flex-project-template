...
const destroySecondDevice = () => {
  ...
  logger.debug('MultiCall: SecondDevice destroyed');
};
...
const handleFlexCallDisconnect = (call: Call) => {
  logger.debug('MultiCall: FlexDeviceCall disconnect', call);
  FlexDeviceCall = null;
...
const handleFlexCallReject = () => {
  logger.debug('MultiCall: FlexDeviceCall reject');
  FlexDeviceCall = null;
...
const handleSecondDeviceRegistered = () => {
  logger.debug('MultiCall: SecondDevice registered');
  ...
  if (speakerDevices && speakerDevices.size > 0) {
    speakerDevices.forEach((speaker) => {
      ...
      try {
        ...
        logger.debug(`MultiCall: Set output device to ${speaker.label}`);
      } catch (error) {
        logger.error('MultiCall: Unable to change output device', error);
      }
    });
  }
};
...
const handleSecondCallIncoming = (call: Call) => {
  ...
  logger.debug('MultiCall: SecondDeviceCall incoming', call);

  call.on('accept', (_innerCall) => {
    logger.debug('MultiCall: SecondDeviceCall accept', call);
  ...
  call.on('disconnect', (_innerCall: Call) => {
    logger.debug('MultiCall: SecondDeviceCall disconnect', call);
...
export const handleFlexCallIncoming = (manager: Manager, call: Call) => {
  logger.debug('MultiCall: FlexDeviceCall incoming');
  ...
  SecondDevice?.on('error', (error) => {
    logger.debug('MultiCall: Error in SecondDevice', error);
  });
