import * as Flex from '@twilio/flex-ui';
import { TaskHelper } from '@twilio/flex-ui';

import { FlexAction, FlexActionEvent } from '../../../../types/feature-loader';
import service from '../../utils/SurveyService';

export const actionEvent = FlexActionEvent.before;
export const actionName = FlexAction.HangupCall;
export const actionHook = async (
  flex: typeof Flex,
  manager: Flex.Manager,
  task: Flex.ITask
) => {
  flex.Actions.addListener(
    `${actionEvent}${actionName}`,
    async (payload, abortFunction) => {
      const { task } = payload;
      if (!TaskHelper.isCallTask(task) || TaskHelper.isOutboundCallTask(task)) {
        return;
      }

      console.log('post-call-survey', task);

      const rules = await service.getRules();

      const surveyRequired = rules.find(
        (r) => r.data.queue_name === task.queueName
      );

      if (surveyRequired) {
        console.log(
          `post-call-survey running survey: ${surveyRequired.data.survey_key}`
        );
        try {
          await service.startSurvey(
            surveyRequired.data.queue_name,
            task.attributes.call_sid,
            task.sid,
            surveyRequired.data.survey_key
          );
        } catch (error) {
          console.log('Error starting survey: ', error);
        }
      }
    }
  );
};
