...
export const hasAnotherNonWorkerJoined = async (task: Flex.ITask) => {
  ...
  if (conference && conference.source) {
    ...
    for (const p of otherNonWorkers) {
      try {
        if (!p.callSid) {
          logger.debug('Unable to get participant from conference, missing participant sid');
          continue;
        }
        ...
        logger.debug('Unable to get participant from conference, it probably ended', error);
      }
    }
...
export const setHangUpBy = (reservationSid: string, value: HangUpBy) => {
  ...
  localStorage.setItem(STORAGE_KEY, JSON.stringify(newValue));
  logger.debug(`Set ${STORAGE_KEY} for ${reservationSid} to ${value}`, newValue);
};
...
export const setHangUpByAttribute = async (
  ...
  try {
    ...
    logger.debug(`Failed to set hang_up_by attribute for ${taskSid} to ${value}`, error);
  }
  logger.debug(`Set hang_up_by attribute for ${taskSid} to ${value}`, newAttributes);
};
...
export const clearHangUpBy = (reservationSid: string) => {
  ...
  if (storage[reservationSid]) {
    ...
    logger.debug(`Removed ${STORAGE_KEY} value for ${reservationSid}`);
  }
};
