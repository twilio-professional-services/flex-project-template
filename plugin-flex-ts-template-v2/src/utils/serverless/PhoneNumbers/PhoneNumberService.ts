...
class PhoneNumberService extends ApiService {
	...
	async getAccountPhoneNumbers(attempts: number): Promise<ListPhoneNumbersResponse> {
		...
		var cachedResult = JSON.parse(localStorage.getItem(this.STORAGE_KEY) || "{}")
		...
		const result = cachedResult.success ? cachedResult : await this.#getAccountPhoneNumbers();
		return result;
	}
	...
	#getAccountPhoneNumbers = (): Promise<any> => {
		...
		return this.fetchJsonWithReject<ListPhoneNumbersResponse>(
			`https://${this.serverlessDomain}/functions/common/flex/phone-numbers/list-phone-numbers`,
			{
				method: 'post',
			...
			if (response.success)
				localStorage.setItem(this.STORAGE_KEY, JSON.stringify({ ...response, expiry: (new Date().getTime() + this.EXPIRY) }));
			return {
				...response,
			};
		});
